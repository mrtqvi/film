...
@section('content')
                ...
                <div class="flex justify-center bg-main rounded col-span-full lg:col-span-6 relative">
                    ...
                    <div
                        ...
                        <div class="flex flex-col justify-start p-6 w-full">
                            ...
                            <a href="{{ route('my-favorite.delete-series' , $series) }}" class="flex text-gray-400 transition-all mt-4 hover:text-red-600 text-sm">
    ...
    @endif
    <div class="grid grid-cols-12 gap-3 px-3 md:px-6 my-5">
        @forelse(auth()->user()->movie as $movies)
            <div class="flex justify-center bg-main rounded col-span-full lg:col-span-6 relative">
                <div class="absolute flex text-xs left-5 top-4 space-x-1 space-x-reverse z-30">
                    @foreach ($movies->categories as $movieCategory)
                        <a href="" class="px-2 rounded-xl py-0.5 bg-slate-800">{{ $movieCategory->name }}</a>
                    @endforeach
                </div>
                <div
                    class="flex flex-wrap sm:flex-nowrap items-center rounded-lg shadow border min-w-full border-gray-800 md:max-w-full md:flex-row relative">
                    <img class="w-full sm:w-44 h-56 object-contain sm:object-cover rounded-3xl p-2 sm:p-4" src="{{ asset($movies->poster) }}" alt="{{ $movies->fa_title }}"/>
                    <span
                        class="dir-ltr text-white border border-slate-50 mt-1 absolute top-2 sm:top-6 right-6 sm:left-6 w-8 h-8 pt-1 px-0.5 rounded-full bg-yellow-500 text-center">+{{ $movies->ages }}</span>
                    <div class="flex flex-col justify-start p-6 w-full">
                        <h5 class="my-2 text-gray-400">
                            {{ $movies->fa_title }} | {{ $movies->en_title }}
                        </h5>
                        <p class="mb-4 text-xs text-gray-500 leading-6">
                            {{ Str::limit($movies->description, 120, '...') }}
                        </p>
                        <div class="flex justify-between">
                            <a href="{{ $movies->show() }}"
                               class="flex text-xs h-8 px-2   items-center  bg-[#64D947] hover:bg-[#00BD08] text-slate-900 transition-all delay-200 py-2  rounded">
                            ...
                            <div class="mr-2 flex items-center mt-1 ml-auto">
                                ...
                                @auth
                                    @if($movies->user->contains(auth()->user()->id))
                                        <section class="add-to-favorite flex justify-start">
                                            <button
                                                data-url="{{ route('series.add-to-favorite' , $movies) }}">
                                                <i class="text-red-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                         stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    @else
                                        <section class="add-to-favorite">
                                            <button   data-url="{{ route('series.add-to-favorite' , $movies) }}">
                                                <i>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                         stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <a href="{{ route('my-favorite.delete-movie' , $movies) }}" class="flex text-gray-400 transition-all mt-4 hover:text-red-600 text-sm">
                            <svg class="ml-1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/>
                            </svg>حذف از لیست علاقه مندی ها
                        </a>
