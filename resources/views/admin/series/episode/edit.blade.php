...
@section('content')
    <div class="row d-flex justify-content-between">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h3 mb-0 section-heading">{{ $episode->title }}</h2>
        </div>
    ...
    @endif
    <form action="{{ route('admin.episodes.update', [$series->id , $episode->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-row">
                            ...
                            <div class="col-12 slug d-flex">
                                <span>https://seriafilm.ir/series/</span>
                            ...
                            <div class="form-group col-md-4">
                                <a href="{{ route('admin.upload-video.index' , ['id' => $episode->id , 'quality' => '1080' , 'model' => get_class($episode)]) }}" target="_blank" type="button" id="browseFile" class="btn btn-outline-dark w-100 @if ($episode->isUploaded('1080')) btn-success @endif)">
                                    ...
                                    <span class="ml-2">آپلود با کیفیت 1080P</span>
                                </a>
                            </div>
                            <div class="form-group col-md-4">
                                <a href="{{ route('admin.upload-video.index' , ['id' => $episode->id, 'quality' => '720' , 'model' => get_class($episode)]) }}" target="_blank" type="button" id="browseFile" class="btn btn-outline-dark w-100 @if ($episode->isUploaded('720')) btn-success @endif">
                                    ...
                                    <span class="ml-2">آپلود با کیفیت 720</span>
                                </a>
                            </div>
                            <div class="form-group col-md-4">
                                <a href="{{ route('admin.upload-video.index' , ['id' => $episode->id, 'quality' => '480' , 'model' => get_class($episode)]) }}" target="_blank" type="button" id="browseFile" class="btn btn-outline-dark w-100 @if ($episode->isUploaded('480')) btn-success @endif">
                                    ...
                                    <span class="ml-2">آپلود با کیفیت 480</span>
                                </a>
                            </div>
                            <div class="form-group col-md-4">
                                <a href="{{ route('admin.upload-video.index' , ['id' => $episode->id , 'quality' => '360' , 'model' => get_class($episode)]) }}" target="_blank" type="button" id="browseFile" class="btn btn-outline-dark w-100 @if ($episode->isUploaded('360')) btn-success @endif">
                                    ...
                                    <span class="ml-2">آپلود با کیفیت 360</span>
                                </a>
                            </div>
                            <div class="form-group col-md-4">
                                <a href="{{ route('admin.upload-video.index' , ['id' => $episode->id,'quality' => '240' , 'model' => get_class($episode)]) }}" target="_blank" type="button" id="browseFile" class="btn btn-outline-dark w-100 @if ($episode->isUploaded('240')) btn-success @endif">
                                    ...
                                    <span class="ml-2">آپلود با کیفیت 240</span>
                                </a>
                            </div>
                        </div>
                    </div> <!-- /. col -->
            ...
            <div class="col-12 col-md-3 my-2 px-0">
                ...
                <div class="card mt-2">
                    ...
                    <div class="card-body">
                        @if ($series->episodes->where('season', $season)->except($episode->id)->count() >= 1)
                        @php $oldSeason = 1; @endphp
                        @while ($season > $oldSeason)
                        <label class="inline-flex items-center">
                            <input type="checkbox" value="{{ $oldSeason }}" name="season"
                                class="form-checkbox border-none ring-none h-5 w-5 text-cyan-700 rounded bg-slate-300 dark:bg-slate-600"
                                @checked(old('season') == $season + 1)><span
                                class="text-sm text-gray-700 dark:text-slate-400 mr-2">
                                اضافه کردن به فصل {{ $oldSeason }}
                            </span>
                        </label>
                        @php $oldSeason++ @endphp
                        @endwhile
                            <label class="inline-flex items-center">
                        ...
                        <div class="main-pic" style="height:8rem !important">
                            ...
                            <img style="height:8rem !important;" src="{{ asset($episode->image) }}" id="output">
                        </div>
                    </div>
